...
func GetQr(c *gin.Context) {
	...
	if req.LogoImage != "" {
		...
		if _, err := os.Stat(filepath); os.IsNotExist(err) {
			fmt.Printf("logo image file %s not found\n", filepath)
			return
		}
	...
	if !req.BgTransparent && req.BgColor != "" {
		rgba, err := convertHexToRGB(req.BgColor)
		if err != nil {
			return
		}
		options = append(options, standard.WithBgColor(rgba))
	}
	...
	// 		return
	// 	}
	// 	options = append(options, standard.WithFgColor(rgba))
	// }
