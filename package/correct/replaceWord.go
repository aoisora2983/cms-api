...
// 文字列置換（not WCAG）
func ReplaceWord(sentence string, status *CorrectStatus, skipIds []int) {
	status.SetKind(constant.REPLACE_WORD)

	words, err := models.GetCorrectTargetWordList()
	if err != nil {
	...
	for _, word := range words {
		...
		if index != -1 && level != constant.CORRECT_OK {
			if level == constant.CORRECT_NG {
				status.SetLevel(constant.CORRECT_NG)
				status.SetMessage(constant.REPLACE_WORD_NG)
				...
				status.SetMessage(constant.REPLACE_WORD_WARNING)
